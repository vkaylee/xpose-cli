# 🐳 Container Infrastructure & Caching Strategy

This project uses a unified container-based workflow to ensure 100% environment parity between local development and CI/CD. Both **Podman** (preferred) and **Docker** are supported via auto-detection.

## 🚀 Unified Workflow (`run-in-container.sh`)

All quality checks are funneled through the `./run-in-container.sh` entry point.
...
> [!NOTE]
> The script auto-detects whether `podman` or `docker` is available on your system. A backward-compatible symlink `run-in-docker.sh` is provided.
...
- **Local (Named Volume)**: We use a Named Volume (`target-cache`).
...
- **Permissions (CI)**: Since the container runtime may run as root, files in `/tmp` are root-owned. `run-in-container.sh` automatically runs `chown -R` at the end of the script in CI mode to restore access to the host runner user.
...
- **Logic**: Executes `./run-in-container.sh lint`.
...
NO_BUILD=true ./run-in-container.sh
```
